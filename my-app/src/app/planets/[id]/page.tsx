    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
      <div className="p-8 border-2 border-foreground rounded-lg">
        ...
        {Object.entries(planet).map(([key, value]) =>
            {
                if(key === 'created' || key === 'edited' || key === 'url' || key === 'films' || key === 'residents') return;
                return(
        ...
        <div key='residents' className="flex justify-between">
          <p className="text-lg mb-2 animate-typing overflow-hidden whitespace-nowrap border-r-4 border-foreground">residents : {residentsList}</p>
        </div>
        <div key='films' className="flex justify-between">
          <p className="text-lg mb-2 animate-typing overflow-hidden whitespace-nowrap border-r-4 border-foreground">films : {filmsList}</p>
        </div>
        </div>
    </div>
  );
